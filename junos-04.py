...
passw = "" # Password for our encrypted RSA key
rsa_key_file = "id_rsa" #The specific private key in the  "~/.ssh/" directory we need to access the far end host
...
#I'm assuming the key is in the "~/.ssh/" directory. 
def decrypt_rsa_key(keyfile, password):
    try:
        #I have a few keys, so just a little double checking to be sure I'll be using the right one.
        print "Is this the key to log in to the remote host? %s" % (keyfile,)
        prompt = raw_input("y/n: ")
        if prompt == "n":
            print "Which key in your ~/.ssh/ directory should I use?"
            keyfile = raw_input("Just the filename please: ")
        #Grabbing the user's home directory and building the full path to the private key.
        home_dir = os.path.expanduser('~')
        keyfile = home_dir + "/.ssh/"+ keyfile
        #Read the contents of the private key file and store it.
        private_key_file = open(keyfile, 'r')
        private_key = private_key_file.read()
        private_key_file.close()
        #Convert the contents of the private key into a string.
        private_key_string = StringIO.StringIO(private_key)
        #Get the password from the user without echo.
        #Should probably add some form of password verification. 
        passw = getpass.getpass(prompt = "Please enter the password for %s :" % (keyfile,))
        ...
        #We've already used the password for our needs, now lets clear it. 
        passw = "################" 
        #Return the decrypted key string.
    except:
        print "Could not decrypt the key, please verify the file/password combination."
        decrypted_key = False
...
try:
    ...
    #Initiate the connection to the bastion/jump box:
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(bastion_host, username=user, pkey=rsa_key)

    #We don't need to store the decrypted rsa key any more. 
    rsa_key= "############"
